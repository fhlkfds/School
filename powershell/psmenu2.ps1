...
# Function to display the main menu (updated)
function Show-MainMenu
{
    Clear-Host
    Write-Host "==========================================" -ForegroundColor Cyan
...
# Function to get device serial number by asset ID with improved error handling
...
# Function to monitor security reports with corrected filter syntax
function Monitor-SecurityReports
{
    ...
    # Build the filter string - fixing the syntax issue
    $dateFilter = "date >= $startDateStr date <= $endDateStr"
    ...
    try
    {
        if ($saveToFile -eq "y")
        {
            ...
            # Execute GAM command with appropriate parameters
            if (-not [string]::IsNullOrWhiteSpace($userEmail))
            {
                # For specific user
                # Note: Using separate parameters for user and date filter to avoid syntax issues
                & gam report $reportType user $userEmail filter "$dateFilter" > $outputPath
            } else
            {
                # For all users
                & gam report $reportType filter "$dateFilter" > $outputPath
            }
        ...
        {
            # Execute GAM command and display results
            if (-not [string]::IsNullOrWhiteSpace($userEmail))
            {
                # For specific user
                Write-Host "Running: gam report $reportType user $userEmail filter `"$dateFilter`"" -ForegroundColor Blue
                & gam report $reportType user $userEmail filter "$dateFilter" | Out-Host
            } else
            {
                # For all users
                Write-Host "Running: gam report $reportType filter `"$dateFilter`"" -ForegroundColor Blue
                & gam report $reportType filter "$dateFilter" | Out-Host
            }
        }
    } catch
    {
